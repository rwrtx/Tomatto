...
checking_sc() {
    ...
    if [[ $date_list < $useexp ]]; then
        ...
        echo -e "     \033[0;36mTelegram${NC}: https:t.me/R32WRT_STORE"
        echo -e "$COLOR1╰═════════════════════════════════════════════════╯${NC}"
        exit
    fi
}
...
Exp=$(curl -sS https://raw.githubusercontent.com/rwrtx/vvipsc/main/izin | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
# DNS PATCH
#tipeos2=$(uname -m)
Name2=$(curl -sS https://raw.githubusercontent.com/rwrtx/vvipsc/main/izin | grep $MYIP | awk '{print $2}')
# GETTING DOMAIN NAME
